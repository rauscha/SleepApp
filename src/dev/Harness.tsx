// Engine harness — the dev surface for poking the audio engine directly
// (synth bed, tinnitus matcher/mask, crossfade demo, scene loader, master).
//
// Extracted from App.tsx and lazy-loaded so its ~700 lines + its
// dev-only imports (NoiseGenerator, TinnitusMaskLayer, ToneMatcher,
// FileLayer, generateTestPadBuffer) are split out of the main bundle
// entirely. The route that mounts it is gated on import.meta.env.DEV, so a
// production build never even references this chunk.
...
export function Harness({ onBackToTonight }: { onBackToTonight: () => void }) {
